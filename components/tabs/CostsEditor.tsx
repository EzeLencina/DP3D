    ...
    return (
        <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <Input label="Costo Electricidad ($/kWh)" id="electricityCostKwh" name="electricityCostKwh" type="number" value={costs.electricityCostKwh === 0 ? '' : costs.electricityCostKwh} onChange={handleChange} icon="electric_meter" unit="$" placeholder="0" />
                ...
                <div>
                    <Input label="% de Ganancia" id="washerProfitPercent" name="washerProfitPercent" type="number" min="0" max="100" value={costs.washerProfitPercent === 0 ? '' : String(costs.washerProfitPercent)} onChange={e => updateCosts({ washerProfitPercent: e.target.value === '' ? '' : parseFloat(e.target.value) })} icon="percent" unit="%" placeholder="0" />
                </div>

                <h3 className="text-lg font-semibold text-white mt-4 md:col-span-2 lg:col-span-3">Costos de Purga</h3>
                <Input label="Purga de filamento (mm)" id="purgeMm" name="purgeMm" type="number" value={costs.purgeMm === 0 ? '' : costs.purgeMm} onChange={handleChange} icon="recycling" unit="mm" placeholder="0"/>
                ...
                <Input label="Diámetro (mm)" id="filamentDiameter" name="filamentDiameter" type="number" step="0.01" value={costs.filamentDiameter === 0 ? '' : costs.filamentDiameter} onChange={handleChange} icon="straighten" unit="mm" placeholder="0" />
                <Input label="Densidad (g/cm³)" id="filamentDensity" name="filamentDensity" type="number" step="0.01" value={costs.filamentDensity === 0 ? '' : costs.filamentDensity} onChange={handleChange} icon="density_medium" unit="g/cm³" placeholder="0" />
