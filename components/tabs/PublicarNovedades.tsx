...
interface PublicarNovedadesProps {
  user: { email: string; admin?: boolean };
}

const PublicarNovedades: React.FC<PublicarNovedadesProps> = ({ user }) => {
            ...
            return (
              <li key={update.id} className="flex items-start gap-2">
                <div className="flex-1">
                  <div className="font-bold text-brand-accent-400 text-lg mb-3" style={{ fontFamily: 'DM Sans, sans-serif' }}>{update.title}</div>
                  <pre className="whitespace-pre-wrap break-words text-slate-200 mb-1" style={{ fontFamily: 'DM Sans, sans-serif' }}>{update.description}</pre>
...
export default PublicarNovedades;
